...
async function saveTask() {
  if (id) {
    editTaskAction({ id, title: title.value, time: time.value, project: project.value });
  } else {
    createTaskAction({ title: title.value, time: time.value, project: project.value });
  }
...
<template>
  <form class="flex flex-col flex-wrap items-center justify-center gap-3" @submit.prevent="saveTask">
    ...
    />
    <select v-model="project" data-test="link-project" class="select select-bordered w-full max-w-md">
      <option value="" selected>Sem projeto</option>
      <option v-for="savedProject of savedProjects" :key="savedProject.id" :value="savedProject.id">
        {{ savedProject.name }}
      </option>
    </select>
